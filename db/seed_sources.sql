...
('deepswe',      'DeepSWE',                     'https://deepswe.datacurve.ai/artifacts/v1.1/leaderboard-live.json',          'A', 'none',    'maintained', 'DataCurve long-horizon coding-agent benchmark. Static JSON rows include model/config, pass@1, attempts, costs, tokens, steps, and confidence intervals. Methodology and tasks at github.com/datacurve-ai/deep-swe.'),
('openvlm',      'OpenCompass OpenVLM',        'http://opencompass.openxlab.space/assets/OpenVLM.json',                      'A', 'none',    'maintained', 'Multimodal/VLM aggregate leaderboard from OpenCompass OpenVLM JSON. Parse model rows and nested benchmark_results with source links.'),
('bigcodebench', 'BigCodeBench results',        'https://datasets-server.huggingface.co/rows?dataset=bigcode%2Fbigcodebench-results&config=default&split=train', 'A', 'none', 'maintained', 'Coding aggregate leaderboard from Hugging Face datasets-server rows for bigcode/bigcodebench-results. Parse complete/instruct benchmark_results with row provenance.'),
...
('vllm',        'vLLM model registry',       'https://raw.githubusercontent.com/vllm-project/vllm/main/vllm/model_executor/models/registry.py', 'A', 'none', 'per-release', 'Supported architecture registry parsed from source text without executing remote Python.'),
