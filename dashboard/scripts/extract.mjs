...
const DB_PATH = resolve(REPO_ROOT, process.env.MODELDB_EXTRACT_DB ?? 'db/modeldb.sqlite')
const OUT_DIR = resolve(REPO_ROOT, process.env.MODELDB_EXTRACT_OUT ?? 'dashboard/public/data')
...
// breaks ties within one provenance class. Recency uses an effective date —
// measured_at, else snapshot fetched_at, else model release_date — so NULL-
// measured newest rows do not lose the pick to older dated rows.
// See viz/README.md.
 ...
 `SELECT model_id, benchmark_id, score, metric, rank, self_reported, measured_at
   FROM (
     SELECT br.model_id, br.benchmark_id, br.score, br.metric, br.rank,
            br.self_reported, br.measured_at,
       ROW_NUMBER() OVER (
         PARTITION BY br.model_id, br.benchmark_id
         ORDER BY br.self_reported ASC,
                  COALESCE(br.measured_at, ss.fetched_at, m.release_date) DESC
       ) AS rn
     FROM benchmark_result br
     LEFT JOIN source_snapshot ss ON ss.id = br.source_snapshot_id
     LEFT JOIN model m ON m.id = br.model_id
 ...
 `SELECT model_id AS modelId, benchmark_id AS benchmarkId, score, metric, rank,
   ...
   FROM (
     SELECT br.model_id, br.benchmark_id, br.score, br.metric, br.rank,
            ...
            ROW_NUMBER() OVER (
              PARTITION BY br.model_id, br.benchmark_id
              ORDER BY COALESCE(br.measured_at, ss.fetched_at, m.release_date) DESC, br.id DESC
            ) AS rn
     FROM benchmark_result br
     LEFT JOIN source_snapshot ss ON ss.id = br.source_snapshot_id
     LEFT JOIN model m ON m.id = br.model_id
...
const throughputRows = q(
 `SELECT br.model_id AS modelId, m.canonical_slug AS canonicalSlug,
   ...
   ORDER BY COALESCE(br.measured_at, ss.fetched_at, m.release_date) DESC,
            br.source_snapshot_id DESC, br.score DESC`,
 ...
 const results = b.id.startsWith('lmarena_')
  ...
  : q(
   `SELECT br.model_id AS modelId, br.score, br.metric, br.rank,
         ...
         ORDER BY COALESCE(br.measured_at, ss.fetched_at, m.release_date)`,
   [b.id]
  ).map((r) => ({
   ...r,
...
function emitElo(benchmarkId, filename) {
 const rows = q(
  `SELECT br.model_id, br.score, br.rank,
          br.measured_at, ss.fetched_at, m.release_date
     FROM benchmark_result br
     LEFT JOIN source_snapshot ss ON ss.id = br.source_snapshot_id
     LEFT JOIN model m ON m.id = br.model_id
     WHERE br.benchmark_id = ? AND br.score IS NOT NULL
     ORDER BY br.model_id, COALESCE(br.measured_at, ss.fetched_at, m.release_date)`,
  [benchmarkId]
 )
 const perModel = new Map() // modelId -> Map(dateKey -> {t, elo, rank, dateSource})
 for (const r of rows) {
  const dateStr = r.measured_at ?? r.fetched_at ?? r.release_date
  const t = dateStr == null ? NaN : Date.parse(dateStr)
  if (!Number.isFinite(t)) continue // no datable timestamp: never a "null"/NaN point
  let m = perModel.get(r.model_id)
  if (!m) perModel.set(r.model_id, (m = new Map()))
  const dateSource = r.measured_at != null ? 'measured' : r.fetched_at != null ? 'snapshot' : 'release'
  const dateKey = dateStr.slice(0, 10)
  m.set(dateKey, { t, elo: r.score, rank: r.rank, dateSource })
 }
 ...
 for (const [modelId, points] of perModel) {
  ...
  series.push({
   ...
   dateSource: sorted.map((p) => p.dateSource)
  })
...
for (const benchId of timeseriesBenchmarks) {
 ...
 const rows = q(
  `SELECT br.model_id AS modelId, br.score, br.rank, br.measured_at AS measuredAt,
          ss.fetched_at AS fetchedAt, m.release_date AS releaseDate,
          br.source_snapshot_id AS sourceSnapshotId
     FROM benchmark_result br
     LEFT JOIN source_snapshot ss ON ss.id = br.source_snapshot_id
     LEFT JOIN model m ON m.id = br.model_id
 ...
 for (const r of rows) {
  const dateStr = r.measuredAt ?? r.fetchedAt ?? r.releaseDate
  const t = dateStr == null ? NaN : Date.parse(dateStr)
  if (!Number.isFinite(t)) continue // no datable timestamp: never a "null"/NaN bucket
  ...
  if (!existing || currentSnap > existing.sourceSnapshotId) {
   m.set(dateKey, {
    ...
    dateSource: r.measuredAt != null ? 'measured' : r.fetchedAt != null ? 'snapshot' : 'release'
   })
  }
 }
